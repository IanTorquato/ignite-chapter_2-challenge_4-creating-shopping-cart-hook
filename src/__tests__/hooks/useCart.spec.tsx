...
describe('useCart Hook', () => {
  ...
  it('should be able to add a new product', async () => {
    ...
    apiMock.onGet(`stock/${productId}`).reply(200, { id: 3, amount: 2 })
    ...
    const { result, waitForNextUpdate } = renderHook(useCart, { wrapper: CartProvider })

    act(() => { result.current.addProduct(productId) })
  ...
  it('should not be able add a product that does not exist', async () => {
    ...
    const { result, waitFor } = renderHook(useCart, { wrapper: CartProvider })
  ...
  it('should be able to increase a product amount when adding a product that already exists on cart', async () => {
    ...
    apiMock.onGet(`stock/${productId}`).reply(200, { id: 1, amount: 3 })
    apiMock.onGet(`products/${productId}`).reply(200, {
      id: 1,
      image: 'https://rocketseat-cdn.s3-sa-east-1.amazonaws.com/modulo-redux/tenis1.jpg',
      price: 179.9,
  ...
  it('should not be able to increase a product amount when running out of stock', async () => {
    ...
    apiMock.onGet(`stock/${productId}`).reply(200, { id: 2, amount: 1 })
    apiMock.onGet(`products/${productId}`).reply(200, {
      id: 2,
      title: "Tênis VR Caminhada Confortável Detalhes Couro Masculino",
      price: 139.9,
  ...
  it('should be able to remove a product', () => {
    ...
    act(() => { result.current.removeProduct(productId) })
  ...
  it('should be able to update a product amount', async () => {
    ...
    apiMock.onGet(`stock/${productId}`).reply(200, { id: 2, amount: 5 })

    const { result, waitForNextUpdate } = renderHook(useCart, { wrapper: CartProvider })

    act(() => { result.current.updateProductAmount({ amount: 2, productId }) })
  ...
  it('should not be able to update a product that does not exist', async () => {
    ...
    act(() => { result.current.updateProductAmount({ amount: 3, productId }) })
  ...
  it('should not be able to update a product amount to a value smaller than 1', async () => {
    ...
    const { result, waitForValueToChange } = renderHook(useCart, { wrapper: CartProvider })

    act(() => { result.current.updateProductAmount({ amount: 0, productId }) })
